...
async function syncVideoAsset(item) {
    ...
    if (existing && (!bilibiliMid || !isLikelyFallbackFavicon(existing)))
...
function findExistingAsset(dir, fileBase) {
    const match = readdirSync(dir).find(
        (name) => name === fileBase || name.startsWith(`${fileBase}.`),
    );
...
async function findBilibiliAvatar(pageUrl, mid) {
    return (
        (await findBilibiliAvatarFromCard(pageUrl, mid)) ??
        (await findBilibiliAvatarFromPage(pageUrl))
    );
}
...
function isBilibiliFaceUrl(value) {
    try {
        ...
        return (
            /^i\d+\.hdslb\.com$/i.test(parsed.hostname) && parsed.pathname.startsWith("/bfs/face/")
        );
    } catch {
        return false;
    }
...
async function findDeclaredIcon(pageUrl) {
    ...
    return candidates.sort((a, b) => getIconCandidateScore(b) - getIconCandidateScore(a)).at(0)
        ?.href;
}
...
function findTags(html, tagName) {
    ...
    while (true) {
        start = lower.indexOf(needle, start);
        if (start === -1) break;
...
function downloadAsset(url, outDir, fileBase, referer) {
    ...
    return new Promise((resolve) => {
        ...
        child.on("close", (code) => {
            ...
            const extension =
                getExtensionFromHeaders(headers) ?? getExtensionFromUrl(url) ?? ".png";
